...
export default function PageBody() {
        ...
        (item: SiteData, searchTerms: string[]): string | false => {
            ...
            if (selectedSearchType === "Site") {
                ...
                for (const term of searchTerms) {
                    const normalizedTerm = normalizeUrl(term.trim())
                    if (normalizedSite === normalizedTerm || normalizedSite.includes(normalizedTerm)) {
                        return normalizedSite
                    }
                }
            } else {
                // For NCC search, do exact match or contains match
                const normalizedMaNCC = searchField.toLowerCase()
                for (const term of searchTerms) {
                    const normalizedTerm = term.trim().toLowerCase()
                    if (normalizedMaNCC === normalizedTerm || normalizedMaNCC.includes(normalizedTerm)) {
    ...
    const handleSearch = useCallback(
        debounce((value: string) => {
            ...
            allData.forEach((item) => {
                const normalizedValue = itemMatchesSearch(item, searchTerms)
                if (normalizedValue) {
    ...
    const getPriceColumnData = (priceType: PriceType, brand: BrandType) => {
        const suffix = brand === "X" ? "Lio" : ""

        switch (priceType) {
            case "GP":
                return `giaBanGP${suffix}`
            case "Text":
                return `giaBanText${suffix}`
            case "TextHome":
                return `giaBanTextHome${suffix}`
            case "TextHeader":
                return `giaBanTextHeader${suffix}`
            default:
                return `giaBanGP${suffix}`
        }
    }
    ...
    const generateColumns = () => {
        ...
        const columnData = getPriceColumnData(selectedPriceType, selectedBrand)
        baseColumns.push({
            title: `Giá Bán`,
            data: columnData,
            width: 65,
        ...
        const additionalColumns = [
            ...
            {
                title: `LN`,
                data: selectedBrand === "X" ? "loiNhuanGPLio" : "loiNhuanGP",
                width: 40,
                className: "htMiddle",
                renderer: ((
                    instance: Handsontable,
    ...
    const calculateSummary = (data: SiteData[]) => {
        ...
        const getPriceColumn = (type: PriceType, brand: BrandType, field: 'giaBan' | 'giaMua' | 'hoaHong' | 'giaCuoi' | 'loiNhuan') => {
            const suffix = brand === "X" ? "Lio" : ""
        ...
        const giaBanColumn = getPriceColumn(selectedPriceType, selectedBrand, 'giaBan')
        const giaMuaColumn = getPriceColumn(selectedPriceType, selectedBrand, 'giaMua')
        const hoaHongColumn = getPriceColumn(selectedPriceType, selectedBrand, 'hoaHong')
        const giaCuoiColumn = getPriceColumn(selectedPriceType, selectedBrand, 'giaCuoi')
        const loiNhuanColumn = getPriceColumn(selectedPriceType, selectedBrand, 'loiNhuan')
        ...
        data.forEach((item) => {
            ...
            summary[giaBanColumn as keyof SiteData] = (Number.parseFloat(summary[giaBanColumn as keyof SiteData]?.toString() || "0") + convertedGiaBan).toString()
            summary[giaMuaColumn as keyof SiteData] = (Number.parseFloat(summary[giaMuaColumn as keyof SiteData]?.toString() || "0") + convertedGiaMua).toString()
            summary[hoaHongColumn as keyof SiteData] = (Number.parseFloat(summary[hoaHongColumn as keyof SiteData]?.toString() || "0") + convertedHoaHong).toString()
            summary[giaCuoiColumn as keyof SiteData] = (Number.parseFloat(summary[giaCuoiColumn as keyof SiteData]?.toString() || "0") + convertedGiaCuoi).toString()
            summary[loiNhuanColumn as keyof SiteData] = (Number.parseFloat(summary[loiNhuanColumn as keyof SiteData]?.toString() || "0") + convertedLoiNhuan).toString()

            // Collect file URLs
            if (Array.isArray(item.FileNCC)) {
                totalFiles += item.FileNCC.length
                fileUrls = [...fileUrls, ...item.FileNCC]
            } else if (item.FileNCC && item.FileNCC !== "No File") {
                totalFiles += 1
        ...
        summary.FileNCC = totalFiles > 0 ? `File (${totalFiles})` : "No File"
        ...
        <div className="min-h-screen bg-gradient-to-br from-blue-50 to-blue-100 py-6 px-4">
            <div className="max-w-7xl mx-auto">
                <div className="bg-white rounded-xl shadow-xl overflow-hidden border border-blue-100">
                    ...
                    <div className="bg-gradient-to-r from-blue-100 to-blue-100 p-4">
                        <div className="relative mb-4">
                            <textarea
                                placeholder={`Tìm kiếm nhiều ${selectedSearchType === "Site" ? "site" : "mã NCC"}\nMỗi dòng một kết quả hoặc phân cách bằng dấu phẩy, khoảng trắng...`}
                                value={searchTerm}
                    ...
                    {loading ? (
                        ...
                        filteredData.length > 0 ? (
                                        ...
                                        <div className="flex items-center justify-between">
                                            <div className="flex flex-col items-left gap-2">
                                                <div className="flex items-center">
                                                    ...
                                                    <h3 className="text-lg font-bold text-blue-700 mb-1">Kết quả tìm kiếm</h3>
                                                </div>
                                            ...
                                            <div className="flex flex-col gap-2">
                                                <div className="flex items-center justify-end">
                                                    <div className="text-right">
                                                        <p className="text-xs text-gray-500">Bạn có thể nhắn tin theo mẫu hoặc soạn tin nhắn gửi đến ncc</p>
                                                ...
                                                <div className="flex items-center gap-1">
                                                    ...
                                                    {hasDuplicates && (
                                                        <button
                                                            ...
                                                            className={`px-2 py-1 rounded-lg text-sm font-medium transition-all duration-200 shadow-md hover:shadow-lg transform hover:scale-105 ${showDuplicates
                                                                ? "bg-gradient-to-r from-red-500 to-red-600 text-white hover:from-red-600 hover:to-red-700"
                                                                : "bg-gradient-to-r from-orange-500 to-orange-600 text-white hover:from-orange-600 hover:to-orange-700"
                                                                }`}
                                                        >
                                                            {showDuplicates ? "Ẩn site trùng" : `Hiện ${duplicatesCount} site trùng`}
                                                        </button>
                                                    )}
                                                </div>
                                    ...
                                    <div className="mt-8 border-t-2 border-gray-200 pt-6">
                                        <div className="bg-gradient-to-r from-orange-50 to-red-50 border border-orange-200 rounded-xl p-4 mb-4 shadow-sm">
                                            <div className="flex items-center">
                                                ...
                                                <div>
                                                    <h3 className="text-lg font-bold text-gray-800 mb-1">Site trùng lặp</h3>
                            ...
                            <div className="flex flex-col items-center justify-center py-12 px-4">
                                ...
                                <p className="text-sm text-gray-500 text-center max-w-sm">
                                    Không tìm thấy kết quả nào phù hợp với từ khóa "{searchTerm}"
                                </p>
                            </div>
                        )
                    ) : (
                        <div className="flex flex-col items-center justify-center py-12 px-4">
                            ...
                            <p className="text-sm text-gray-500 text-center max-w-sm">
                                Nhập từ khóa để tìm kiếm dữ liệu. Bạn có thể tìm kiếm nhiều site hoặc NCC cùng lúc bằng cách phân cách
                                chúng bằng dấu phẩy.
                            </p>
                        </div>
                    )}
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
                    <div className="bg-white rounded-xl shadow-xl max-w-lg w-full p-6 max-h-[90vh] overflow-y-auto">
                        <div className="flex justify-between items-center mb-4">
                            <h3 className="text-xl font-bold text-gray-800">Gửi tin nhắn trực tiếp</h3>
                            <button
                                onClick={() => {
                                    setShowDirectMessageModal(false)
                                    setDirectMessage("")
            ...
            {showNccSelectionModal && (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
                    <div className="bg-white rounded-xl shadow-xl max-w-2xl w-full p-6 max-h-[90vh] overflow-y-auto">
                        <div className="flex justify-between items-center mb-4">
                            <h3 className="text-xl font-bold text-gray-800">Chọn NCC để gửi tin nhắn</h3>
