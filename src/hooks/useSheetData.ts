...
const CACHE_EXPIRY_KEY = "sheet_data_cache_expiry"
...
/**
 * Hook tối ưu để fetch và cache dữ liệu từ sheet
...
const getCachedDataSync = (): SheetData | null => {
  ...
  try {
    ...
    return JSON.parse(cachedData) as SheetData
  } catch (err) {
    return null
  }
}

export function useSheetData(autoFetch = true): UseSheetDataReturn {
  ...
  const getCachedData = useCallback((): SheetData | null => {
    if (typeof window === "undefined") return null
  ...
  const saveToCache = useCallback((dataToCache: SheetData) => {
  ...
  const fetchData = useCallback(async (showLoading = true, isBackgroundRefresh = false) => {
    try {
      ...
      const response = await sheetApiRequest.getData()
      const sheetData: SheetData = {
        gpTextVN: response.gpTextVN || [],
        gpTextNN: response.gpTextNN || [],
        synthetic: response.synthetic || [],
        content: response.content || [],
      }

      setData(sheetData)
      saveToCache(sheetData)
      setIsStale(false)
    } catch (err) {
