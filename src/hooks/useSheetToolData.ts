...
const CACHE_KEY = "sheet_tool_data_cache"
const CACHE_EXPIRY_KEY = "sheet_tool_data_cache_expiry"
...
/**
 * Hook tối ưu để fetch và cache dữ liệu từ sheet tool
...
const getCachedDataSync = (): ToolData | null => {
  ...
  try {
    ...
    return JSON.parse(cachedData) as ToolData
  } catch (err) {
    return null
  }
}

export function useSheetToolData(autoFetch = true): UseSheetToolDataReturn {
  // Khởi tạo với cached data ngay từ đầu để tránh re-render
  const [data, setData] = useState<ToolData | null>(() => {
  ...
  const getCachedData = useCallback((): ToolData | null => {
    if (typeof window === "undefined") return null
  ...
  const saveToCache = useCallback((dataToCache: ToolData) => {
  ...
  const fetchData = useCallback(async (showLoading = true, isBackgroundRefresh = false) => {
    try {
      ...
      const response = await sheetApiRequest.getDataTool()
      const toolData: ToolData = {
        gpTextVN: response.gpTextVN || [],
      }

      setData(toolData)
      saveToCache(toolData)
      setIsStale(false)
    } catch (err) {
      const error = err instanceof Error ? err : new Error("Unknown error")
      setError(error)
      console.error("Error fetching sheet tool data:", error)
